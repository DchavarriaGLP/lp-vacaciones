...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={cn('inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium', styles[status] ?? 'bg-gray-800 text-gray-400')}>
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
      <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-md p-6 shadow-2xl">
        <h3 className="text-lg font-semibold text-white mb-1">Decisión de aprobación</h3>
        <p className="text-sm text-gray-400 mb-5">{employeeName}</p>
        ...
        <div className="mb-4">
          <label className="block text-sm font-medium text-gray-300 mb-1.5">
            ...
            {decision === 'approved' && <span className="text-gray-500">(opcional)</span>}
          </label>
          <textarea
            value={notes}
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={onClose}
            className="px-4 py-2.5 text-sm text-gray-400 border border-gray-700 rounded-lg hover:text-white transition-colors"
          >
            Cancelar
          </button>
        </div>
...
export function AprobacionesClient({ pendingSteps, allRequests, role }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      {pendingSteps.length > 0 && (
        <section>
          <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wide mb-3">Pendientes de tu aprobación</h2>
      ...
      {pendingSteps.length === 0 && (
        <div className="bg-gray-900 border border-gray-800 rounded-2xl py-16 text-center">
          ...
          <p className="text-gray-500">No tienes solicitudes pendientes de aprobación.</p>
        </div>
      )}
      ...
      {role === 'admin' && allRequests && allRequests.length > 0 && (
        <section>
          <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wide mb-3">Historial de solicitudes (admin)</h2>
                ...
                <tbody className="divide-y divide-gray-800">
                  {(allRequests as Array<{
                    ...
                    <tr key={req.id} className="hover:bg-gray-800/40">
                      <td className="px-4 py-3 text-white">{req.employees?.full_name ?? '—'}</td>
                      <td className="px-4 py-3 text-gray-400 text-xs">{req.employees?.companies?.name ?? '—'}</td>
